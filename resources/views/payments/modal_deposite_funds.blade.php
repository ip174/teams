<div class="modal fade paymentmethodmodal" id="depositfundModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <form action="{{ url('/payments/pay-now') }}" method="post">
        {{ csrf_field() }}
      <div class="modal-header newMheader">
	    	<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
	    	<h2 class="mr0 task_titel">Deposite funds</h2>
	  </div>
      <div class="modal-body">
        <div class="newform clearfix paddlr5">
        	<div class="col-sm-8">
        		<label>To</label>
        		<select class="form-control minimal" name="job_project_id" required>
        			<option value=""> -Select- </option>
                    @if(count($depositeFundsForJob) > 0)
                        @foreach($depositeFundsForJob as $eachJob)
                            <option value="{{ $eachJob->job_id.'-'.$eachJob->proposal_id }}"> {{ $eachJob->needs_to_design_job }} </option>
                        @endforeach
                    @endif
        		</select>
        	</div>
        	<div class="col-sm-4">
        		...
        		<div class="input-group">
        			...
        			<input type="text" class="form-control" placeholder="0.00" name="deposite_amount" required>
        		</div>
        	</div>
        </div>
